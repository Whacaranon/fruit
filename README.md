...
function AutoHaki()
    if not  game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("Enhance"):FireServer() then
    game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("Enhance"):FireServer()
end
end
...
local RenUi = Update:AddWindow("Infinity Hub","10039618734",Enum.KeyCode.RightControl)
    ...
    local M = RenUi:AddTab("Main","6026568198")
        local S = RenUi:AddTab("Stats","6026568198")
        local A = RenUi:AddTab("Miss","")


Time = M:AddLabelLeft("Server Time")
...
SelectWeapona = M:AddDropdownRight("Select Weapon",SelectWeapon,function(a)
    _G.SelectWeapon = a
    end)

M:AddButtonRight("reset Weapon",function()
    SelectWeapona:Clear()
    for i,v in pairs(game:GetService("Players").LocalPlayer.Backpack:GetChildren()) do
...
M:AddToggleRight("Auto Stats Sword-Haki-Mana-Melee",_G.Auto_Stats,function(a)
    _G.Auto_Stats = a
end)
...
game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("Mouse1Combat"):FireServer(unpack(args))
until _G.Auto_Stats == false
end
end)
end
end)
     ...
     M:AddToggleRight("Auto Stats Fruits",_G.Auto_Stats_Fruits,function(a)
         _G.Auto_Stats_Fruits = a
         end)
     ...
     spawn(function()
         while wait() do
             ...
             game:GetService("VirtualInputManager"):SendKeyEvent(true,"V",false,game)
                    ...
                    until _G.Auto_Stats_Fruits == false
                     end
                 end)
end
end)
     
     M:AddToggleRight("Auto Stats Defense",_G.Auto_Stats_Defense,function(a)
         _G.Auto_Stats_Defense = a
         end)
...
Mon = M:AddDropdownLeft("Select Mon",Mobs,function(a)
    _G.SelectMon = a
    end)

M:AddButtonLeft("Reset Mon",function()
    Mon:Clear()
    for i,v in pairs(game:GetService("Workspace").Mobs:GetChildren()) do
        Mon:Add(v.Name)
    end
...
M:AddToggleLeft("Auto Farm",_G.AutoFarm,function(a)
    _G.AutoFarm = a
end)
...
spawn(function()
    while wait() do 
        pcall(function()
            if _G.AutoFarm then
                for i,v in pairs(game:GetService("Workspace").Mobs:GetChildren()) do
                   if v.Name == _G.SelectMon and v:FindFirstChild("Humanoid") and v:FindFirstChild("HumanoidRootPart") and v.Humanoid.Health > 0 then
                    repeat wait()
                        local args = {
    [1] = "GetQuest",
    [2] = _G.SelectMon
...
game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("QuestRemote"):FireServer(unpack(args))
    ...
    M:AddToggleLeft("Auto Cheat",_G.AutoCheat,function(a)
        _G.AutoCheat = a
    end)
...
game:GetService("ReplicatedStorage"):WaitForChild("Remotes"):WaitForChild("ChestsPick"):FireServer(unpack(args))
                    until _G.AutoCheat == false or not v
end
end
end)
end
...
S:AddDropdownLeft("Select Npc",NPC,function(a)
    _G.SelectNpc = a
end)

S:AddButtonLeft("TP To NPC",function()
    for i,v in pairs(game:GetService("Workspace").NPCs:GetChildren()) do
        if v.Name == _G.SelectNpc then
            game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = v.Head.CFrame
             game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame = v.Hendle.Position
        end
    end
end)

 A:AddButtonLeft("Delete Fog",function(a)
